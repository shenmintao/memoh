...
- `/runtimes/status` 使用设备自己的 Runtime Key 返回该设备的服务端连接状态，供托盘与电脑列表保持一致；凭据失效与暂时无法查询分别处理。
- [通用消息推送](message-push.md)：每个外部来源使用独立密钥，直接向自己的关联渠道账号转发通知，兼容短信转发器 POST JSON；支持暂停、重置密钥、去重与发送结果查询。
...
通用推送涉及数据库迁移 0149，须同时更新 Server、Channel 和 Web。升级前备份数据库，先在副本验证 148 → 149；外层反向代理也应关闭 `/api/push/` 的完整 URL 日志。若使用压缩包打包静态资源，确保镜像中静态目录可由 Nginx 工作进程遍历、文件可读。

2026-09-11 通用推送已完成迁移副本预演、相关 Go 测试与 race 检查、Go lint、Web 生产构建、SDK 类型检查和实际微信投递验证。Bearer 与 URL 密钥、无效密钥、暂停、轮换、去重及删除均已验证；用户确认管理页面正常且通知已收到。Web 全量类型检查和 UI contract 仍有已有问题，本次新增组件未产生对应检查错误。
