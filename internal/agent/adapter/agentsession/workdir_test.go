...
type fakeThreadGetter struct{ item thread.Thread }

func (f fakeThreadGetter) Get(context.Context, string) (thread.Thread, error) { return f.item, nil }
func (f fakeThreadGetter) MergeRuntimeMetadata(context.Context, string, string, map[string]any) (thread.Thread, error) {
	return f.item, nil
}
